...
MSA_Unemployment = pd.read_csv('RAW_ESRI_MSA_unemployment.csv',converters={'Geo_ID':str}).drop(columns=['Unnamed: 0'])
...
match = pd.merge(MSA_Unemployment,bls_unemployment_data, how='left', left_on=['Geo_ID'], right_on=['Geo_ID']).rename(columns={'UnemploymentRate_x': 'UnemploymentRate_ESRI', 'UnemploymentRate_y': 'UnemploymentRate_BLS'})


# Make sure there are no new missing ESRI IDs that we have not accounted for
arcgisids_not_in_BLS = MSA_Unemployment[(~MSA_Unemployment.Geo_ID.isin(match.Geo_ID))]
